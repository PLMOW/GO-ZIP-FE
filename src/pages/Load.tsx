...
const Load = () => {
    ...
    return (<div style={{
        marginTop: '95px', display: 'flex', flexDirection: 'column', alignItems:
            'center', padding: '0px 50px'
    }}>
        <form onSubmit={(e) => {
            e.preventDefault();
            ...
            <InputWrapper >
                <Input value={data.title} onChange={onChangeTitleHandler} placeholder='제목을 입력해주세요'></Input>
            </InputWrapper>
            ...
            <div style={{
                ...
                {structure.map((item, i) => {
                    return <Btn key={i} onClick={() => {
                        setData({
                            ...data, house_type: item,
                        });

                    }} type='button'>{item}</Btn>
                })}


            </div>
            <TextArea placeholder='해당 방에 대한 상세정보를 입력해주세요.' spellCheck='false' value={data.description} onChange={onChangeDescHandler}></TextArea>
            <div style={{ display: 'flex', margin: '20px 0', gap: '40px' }}>

                <Btn>등록하기</Btn>
                <Btn onClick={() => {
                    navigate('/');
                }} type='button'>취소하기</Btn>
            </div>
